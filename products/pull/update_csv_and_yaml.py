...
def fetch_epds(state: str, authorization) -> list:
    ...
    try:
        ...
        for page in range(1, total_pages + 1):
            ...
            selective_responses.append(page_data)
...
def create_folder_path(state, zipcode, display_name):
    ...
    if zipcode and len(str(zipcode)) >= 5:
        return os.path.join("US", state_code, zipcode[:2], zipcode[2:], display_name)
    else:
        return os.path.join("US", state_code, "unknown", display_name)
...
def save_json_to_yaml(state: str, json_data: list):
    ...
    for epd in filtered_data:
        try:
            ...
            display_name = epd.get('category', {}).get('display_name', 'unknown')
            if display_name:
            ...
            if not material_id or material_id == 'unknown':
                # Use open_xpd_uuid as fallback
                material_id = epd.get('open_xpd_uuid', 'unknown')
            ...
            logging.error(f"Error saving YAML for {state}: {str(e)}")
            continue

def map_response(epd: dict) -> dict:
    try:
        dict_attributes = {
            'Category_epd_name': epd.get('category', {}).get('openepd_name', ''),
            'Name': epd.get('name', ''),
            'ID': epd.get('open_xpd_uuid', ''),
            'Zip': epd.get('plant_or_group', {}).get('postal_code', ''),
            'County': epd.get('plant_or_group', {}).get('admin_district2', ''),
            'Address': epd.get('plant_or_group', {}).get('address', ''),
            'Latitude': epd.get('plant_or_group', {}).get('latitude', ''),
            'Longitude': epd.get('plant_or_group', {}).get('longitude', '')
        }
    ...
    if authorization:
        for state in states:
            try:
                ...
                if results:
                    print(f"Got {len(results)} results for {state}")
